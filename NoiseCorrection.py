...
class NoiseCorrection:
    ...
    def calculate_noise(self):
        ...
        for m in range(self.M):
            ...
            for train_index, test_index in kf.split(self.X):
                ...
                p = Process(target=self.train_and_eval, args=(train_index, test_index))
                p.start()
                jobs.append(p)
    ...
    def get_noise_index(self, fracion=1.0):
        all_index = range(len(self.y))
        sorted_index = [x for _,x in sorted(zip(self.r, all_index), reverse=True)]
        return sorted_index[0:int(self.theta * fracion)]

    def train_and_eval(self, train_index, test_index):
