...
class NoiseCorrection:
    ...
    def calculate_noise(self):
        ...
        for m in range(self.M):
            ...
            kmeans = KMeans(n_clusters=int(self.K / 2)).fit(self.X)
            rand = np.random.rand(len(self.y))
            for i in range(self.K):
                train_index = all_index[np.logical_and(kmeans.labels_ != i, rand > 0.5)]
                test_index = all_index[np.logical_and(kmeans.labels_ == i, rand > 0.5)]
                if len(test_index) > 0:
